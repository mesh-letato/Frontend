...
export function SpaceHeaderTop({ space, mine = false }) {
  ...
  return (
    <>
      ...
      <div style={{ padding: '12px 20px 0', font: '800 28px/1 Pinmoa, system-ui', letterSpacing: '-1px', color: '#fff' }}>{space?.name || '성수 맛집 🍝'}</div>
      <div style={{ padding: '9px 20px 0', display: 'flex', alignItems: 'center', gap: 9 }}>
        {mine ? (
          <>
            <Avatar m={members.me} size={26} border="#0D0D0F" />
            <span style={{ font: '600 13px/1 Pinmoa, system-ui', color: 'rgba(255,255,255,.55)' }}>{placeCount}곳 · 나만의 지도</span>
          </>
        ) : (
          <>
            <div style={{ display: 'flex' }}>
              {['me', 'jiyoon', 'doyoon'].map((k, i) => <Avatar key={k} m={members[k]} size={26} border="#0D0D0F" ml={i === 0 ? 0 : -9} />)}
              <div style={{ width: 26, height: 26, borderRadius: '50%', border: '2px solid #0D0D0F', background: '#2a2a2e', marginLeft: -9, display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 9px/1 Pinmoa, system-ui', color: '#fff' }}>+2</div>
            </div>
            <span style={{ font: '600 13px/1 Pinmoa, system-ui', color: 'rgba(255,255,255,.55)' }}>{placeCount}곳 · 멤버 {space?.memberCount ?? 5}</span>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 130, background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={onClose}>
      <div className="pm-up" onClick={(e) => e.stopPropagation()} style={{ background: '#1a1a1e', borderRadius: '28px 28px 0 0', padding: '22px 20px 40px', maxHeight: '82%', overflowY: 'auto' }}>
        ...
        <div style={{ font: '800 23px/1 Pinmoa, system-ui', letterSpacing: '-.7px', color: '#fff', marginBottom: 6 }}>멤버 관리</div>
        <div style={{ font: '500 13.5px/1.5 Pinmoa, system-ui', color: 'rgba(255,255,255,.5)', marginBottom: 20 }}>참가코드를 친구에게 보내면 이 스페이스에 초대할 수 있어요</div>

        {/* 참가코드 + 복사 */}
        <div style={{ font: '700 12px/1 Pinmoa, system-ui', letterSpacing: '.3px', color: '#6a6a70', marginBottom: 10 }}>참가 코드</div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, background: '#0D0D0F', borderRadius: 16, padding: '15px 16px', marginBottom: 24 }}>
          <span style={{ font: '800 21px/1 Pinmoa, system-ui', letterSpacing: '2px', color: '#fff' }}>{code}</span>
          <div className="pm-tap" onClick={copy} style={{ display: 'flex', alignItems: 'center', gap: 6, background: copied ? 'rgba(48,209,88,.16)' : '#2997ff', borderRadius: 9999, padding: '9px 14px' }}>
            {copied
              ...
              : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="9" y="9" width="11" height="11" rx="2.5" stroke="#fff" strokeWidth="1.9" /><path d="M5 15V6a2 2 0 0 1 2-2h9" stroke="#fff" strokeWidth="1.9" strokeLinecap="round" /></svg><span style={{ font: '800 12px/1 Pinmoa, system-ui', color: '#fff' }}>코드 복사</span></>}
          </div>
        </div>

        {/* 멤버 목록 */}
        <div style={{ font: '700 12px/1 Pinmoa, system-ui', letterSpacing: '.3px', color: '#6a6a70', marginBottom: 12 }}>멤버 {memberKeys.length}</div>
            ...
            return (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 2px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ font: '700 15px/1.1 Pinmoa, system-ui', color: '#fff' }}>{m.name}{k === 'me' && <span style={{ marginLeft: 6, font: '600 11px/1 Pinmoa, system-ui', color: '#6a6a70' }}>나</span>}</div>
                  <div style={{ marginTop: 3, font: '500 12px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{i === 0 ? '방장' : '멤버'}</div>
                </div>
              </div>
            );
          })}
