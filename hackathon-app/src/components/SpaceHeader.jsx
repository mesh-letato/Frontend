...
export function SpaceHeaderTop({ space, mine = false, placeCount: placeCountProp }) {
  ...
  return (
    <>
      ...
      <div style={{ padding: '12px 20px 0', font: '800 28px/1 Pinmoa, system-ui', letterSpacing: '-1px', color: '#fff' }}>{space?.emoji ? `${space.emoji} ` : ''}{space?.name || '스페이스'}</div>
        ...
        {mine ? (
          ...
          <>
            <div style={{ display: 'flex' }}>
              ...
              {extra > 0 && (
                <div style={{ width: 26, height: 26, borderRadius: '50%', border: '2px solid #0D0D0F', background: '#2a2a2e', marginLeft: -9, display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 9px/1 Pinmoa, system-ui', color: '#fff' }}>+{extra}</div>
              )}
            </div>
            <span style={{ font: '600 13px/1 Pinmoa, system-ui', color: 'rgba(255,255,255,.55)' }}>{placeCount}곳 · 멤버 {memberCount}명</span>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 130, background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={onClose}>
      <div className="pm-up" onClick={(e) => e.stopPropagation()} style={{ background: '#1a1a1e', borderRadius: '28px 28px 0 0', padding: '22px 20px 40px', maxHeight: '82%', overflowY: 'auto' }}>
        ...
        {/* 멤버 목록 */}
        <div style={{ font: '700 12px/1 Pinmoa, system-ui', letterSpacing: '.3px', color: '#6a6a70', marginBottom: 12 }}>멤버 {members.length}</div>
            ...
            return (
              <div key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 2px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ font: '700 15px/1.1 Pinmoa, system-ui', color: '#fff' }}>{m.nickname}</div>
                  <div style={{ marginTop: 3, font: '500 12px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{isOwner ? '방장' : '멤버'}</div>
                </div>
              </div>
            );
          })}
