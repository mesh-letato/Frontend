    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0D0D0F', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(48,209,88,.15)', borderRadius: 9999, padding: '6px 12px' }}>
          ...
          <span style={{ font: '700 12px/1 Pinmoa, system-ui', color: '#30d158' }}>릴스에서 장소 확인됨</span>
        </div>
        <div style={{ width: 36, height: 36 }} />
      </div>

      <div className="pm-scroll" style={{ flex: 1, paddingBottom: 150 }}>
        <div style={{ padding: '16px 20px 0', font: '800 26px/1.2 Pinmoa, system-ui', letterSpacing: '-.8px', color: '#fff' }}>어디에 저장할까요?</div>
        <div style={{ padding: '6px 20px 0', font: '500 14px/1.35 Pinmoa, system-ui', color: 'rgba(255,255,255,.5)' }}>이 장소를 추가할 스페이스를 골라주세요</div>
        ...
        <div style={{ margin: '16px 20px 0', display: 'flex', gap: 13, background: '#18181B', borderRadius: 20, padding: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ font: '800 17px/1.1 Pinmoa, system-ui', color: '#fff' }}>{p.name}</div>
            <div style={{ marginTop: 5, font: '500 12px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{shortCat(p.category)} · {p.roadAddress || p.address}</div>
            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 5 }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><rect x="2" y="2" width="20" height="20" rx="6" stroke="#2997ff" strokeWidth="2" /><circle cx="12" cy="12" r="4.5" stroke="#2997ff" strokeWidth="2" /></svg><span style={{ font: '600 11px/1 Pinmoa, system-ui', color: '#2997ff' }}>릴스에서 가져옴</span></div>
          </div>
        </div>

        <div style={{ padding: '20px 20px 6px', font: '700 12px/1 Pinmoa, system-ui', letterSpacing: '.3px', color: '#6a6a70' }}>스페이스 선택 ({picked.size})</div>
          ...
          {allSpaces.map((s) => {
            ...
            return (
              <div key={s.id} className="pm-tap" onClick={() => !s.locked && toggle(s.id)}
                ...
                <div style={{ width: 44, height: 44, borderRadius: 13, background: on ? '#2997ff' : '#0D0D0F', display: 'flex', alignItems: 'center', justifyContent: 'center', font: '400 20px/1 Pinmoa, system-ui' }}>{s.emoji}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                    <span style={{ font: '700 15.5px/1.1 Pinmoa, system-ui', color: '#fff' }}>{s.name}</span>
                    {s.locked && <span style={{ font: '700 10px/1 Pinmoa, system-ui', color: '#2997ff', background: 'rgba(41,151,255,.16)', borderRadius: 9999, padding: '3px 7px' }}>기본</span>}
                  </div>
                  <div style={{ marginTop: 4, font: '500 12px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{s.sub}</div>
                </div>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '14px 20px 30px', background: 'linear-gradient(180deg,rgba(13,13,15,0),#0D0D0F 38%)' }}>
        {error && <div style={{ marginBottom: 10, font: '600 12.5px/1.4 Pinmoa, system-ui', color: '#ff7a7a' }}>{error}</div>}
        <div className="pm-tap" onClick={confirm} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#fff', borderRadius: 18, padding: 17, boxShadow: '0 10px 24px rgba(0,0,0,.4)', opacity: saving || picked.size === 0 ? 0.6 : 1 }}>
          ...
          <span style={{ font: '800 16px/1 Pinmoa, system-ui', color: '#0D0D0F' }}>{saving ? '저장 중…' : '확인'}</span>
        </div>
      </div>
      <HomeIndicator />
    </div>
