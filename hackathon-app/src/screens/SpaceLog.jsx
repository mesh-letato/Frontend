    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0D0D0F', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0 }}>
        ...
        <div style={{ padding: '16px 20px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ font: '600 11.5px/1 Pinmoa, system-ui', color: '#6a6a70' }}>후기 {logs.length}개</span>
        </div>
      </div>
      ...
      <div className="pm-scroll" style={{ flex: 1, padding: '0 20px 30px' }}>
        {loading ? (
          ...
          <div style={{ margin: '24px 0', padding: '28px 20px', background: '#18181B', borderRadius: 22, textAlign: 'center' }}>
            <div style={{ font: '700 15px/1.4 Pinmoa, system-ui', color: '#fff' }}>아직 올라온 후기가 없어요</div>
            <div style={{ marginTop: 8, font: '500 13px/1.5 Pinmoa, system-ui', color: '#6a6a70' }}>장소에 다녀온 후기를 남겨보세요 📸</div>
          </div>
        ) : (
    ...
    <div style={{ background: '#18181B', borderRadius: 22, padding: 13 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: '50%', padding: 2, background: 'conic-gradient(from 210deg,#f0a868,#d76a8a,#a83a5a,#f0a868)', flexShrink: 0 }}>
          <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '2px solid #18181B', background: memberColor(log.userId), display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 12px/1 Pinmoa, system-ui', color: '#fff' }}>{memberInitial(log.nickname)}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ font: '700 14px/1 Pinmoa, system-ui', color: '#fff' }}>{log.nickname || '회원'}</span>
            <span style={{ font: '500 11px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{timeAgo(log.createdAt)}</span>
          </div>
          <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ font: '500 11px/1 Pinmoa, system-ui', color: '#6a6a70' }}>다녀왔어요</span>
          </div>
        </div>
      </div>
      {/* 폴라로이드 + 한 줄 후기 */}
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        <div style={{ width: 88, background: '#fff', padding: '5px 5px 0', borderRadius: 3, transform: `rotate(${rot}deg)`, boxShadow: '0 8px 22px rgba(0,0,0,.55)', flexShrink: 0 }}>
          ...
          <div style={{ padding: '5px 3px 9px', font: "700 10.5px/1.2 Pinmoa, system-ui", color: '#333', textAlign: 'center', overflow: 'hidden' }}>{clip(log.content)}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
          <div style={{ font: "700 17px/1.3 Pinmoa, system-ui", color: '#fff' }}>{log.content}</div>
        </div>
      </div>
    </div>
  );
