    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0D0D0F', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0 }}>
        ...
        <div style={{ padding: '16px 20px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ font: '700 12px/1 Pinmoa, system-ui', letterSpacing: '.3px', color: '#6a6a70' }}>최신순</span>
          <span style={{ font: '600 11.5px/1 Pinmoa, system-ui', color: '#6a6a70' }}>후기 {spaceLogs.length}개</span>
        </div>
      </div>
    ...
    <div className="pm-tap" onClick={onTap} style={{ background: '#18181B', borderRadius: 22, padding: 13 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: '50%', padding: 2, background: 'conic-gradient(from 210deg,#f0a868,#d76a8a,#a83a5a,#f0a868)', flexShrink: 0 }}>
          <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '2px solid #18181B', background: who.color, display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 12px/1 Pinmoa, system-ui', color: '#fff' }}>{who.initial}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ font: '700 14px/1 Pinmoa, system-ui', color: '#fff' }}>{who.name}</span>
            <span style={{ font: '500 11px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{log.when}</span>
          ...
          <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ font: '700 11.5px/1 Pinmoa, system-ui', color: '#2997ff' }}>{log.place}</span>
            <span style={{ font: '500 11px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{log.action}</span>
          </div>
        </div>
      </div>
      {/* 폴라로이드 + 정보 */}
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        <div style={{ width: 88, background: '#fff', padding: '5px 5px 0', borderRadius: 3, transform: `rotate(${log.rot}deg)`, boxShadow: '0 8px 22px rgba(0,0,0,.55)', flexShrink: 0 }}>
          ...
          <div style={{ padding: '5px 3px 9px', font: "700 10.5px/1.2 Pinmoa, system-ui", color: '#333', textAlign: 'center', overflow: 'hidden' }}>{clip(log.caption)}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(41,151,255,.12)', borderRadius: 9999, padding: '5px 9px' }}>
            <span style={{ font: '600 11px/1 Pinmoa, system-ui', color: '#2997ff' }}>{log.area}</span>
          </div>
          <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ display: 'flex' }}>
              {log.wantAvatars.map((k, i) => (
                <div key={k} style={{ width: 18, height: 18, borderRadius: '50%', background: members[k].color, border: '1.5px solid #18181B', marginLeft: i === 0 ? 0 : -5, display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 7px/1 Pinmoa, system-ui', color: '#fff' }}>{k === 'me' ? '나' : ''}</div>
              ))}
            </div>
            <span style={{ font: '500 11px/1.3 Pinmoa, system-ui', color: '#6a6a70' }}>{log.wantBy}</span>
          </div>
        </div>
      </div>
    </div>
