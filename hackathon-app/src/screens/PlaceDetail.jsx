      ...
      <div className="pm-scroll" style={{ position: 'absolute', top: 178, left: 0, right: 0, bottom: 0 }}>
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <div>
              <div style={{ font: '800 26px/1 Pinmoa, system-ui', letterSpacing: '-.7px', color: '#fff' }}>{place.name}</div>
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ font: '700 14px/1 Pinmoa, system-ui', color: '#fff' }}>⭐ {place.rating || '4.8'}</span>
                <span style={{ font: '500 13px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{place.cat} · {place.area}</span>
              </div>
            </div>
          ...
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="pm-tap" onClick={() => showToast(`${place.count || 5}명이 저장: 나, 지윤, 도윤…`)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#18181B', borderRadius: 13, padding: '9px 12px' }}>
              ...
              <span style={{ font: '700 12px/1 Pinmoa, system-ui', color: '#fff' }}>{place.count || 5}명 저장</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#18181B', borderRadius: 13, padding: '9px 12px' }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: '#30d158' }} /><span style={{ font: '700 12px/1 Pinmoa, system-ui', color: '#fff' }}>영업중 · 22시까지</span></div>
          </div>
          <div style={{ marginTop: 11, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ font: '600 13px/1 Pinmoa, system-ui' }}>📍</span><span style={{ font: '500 13px/1.2 Pinmoa, system-ui', color: 'rgba(255,255,255,.7)' }}>{place.addr || '서울 성동구 연무장길 33 · 320m'}</span></div>
        </div>

        {/* 릴스 */}
        <div style={{ marginTop: 24, padding: '0 20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            <div style={{ font: '800 18px/1 Pinmoa, system-ui', letterSpacing: '-.4px', color: '#fff' }}>이 장소를 저장한 릴스 ▶️</div>
            <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 5 }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M7 17L17 7M17 7H9M17 7v8" stroke="#6a6a70" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg><span style={{ font: '500 11.5px/1.3 Pinmoa, system-ui', color: '#6a6a70' }}>탭하면 인스타에서 열려요</span></div>
          </div>
          <span style={{ font: '800 13px/1 Pinmoa, system-ui', color: '#2997ff' }}>{reels.length}</span>
        </div>
          ...
          {reels.map((r, i) => (
            <div key={i} className="pm-tap" onClick={openReel} style={{ position: 'relative', width: 139, height: 200, flexShrink: 0, borderRadius: 18, overflow: 'hidden', background: r.g, boxShadow: '0 10px 24px rgba(0,0,0,.45)' }}>
              ...
              <div style={{ position: 'absolute', left: 11, right: 11, bottom: 11 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 19, height: 19, borderRadius: '50%', background: 'linear-gradient(140deg,#f9ce7a,#e0683e)', border: '1.5px solid #fff' }} /><span style={{ font: '800 12px/1 Pinmoa, system-ui', color: '#fff' }}>{r.user}</span></div>
                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 5 }}><svg width="11" height="11" viewBox="0 0 24 24"><path d="M8 5v14l11-7L8 5Z" fill="rgba(255,255,255,.85)" /></svg><span style={{ font: '600 10.5px/1 Pinmoa, system-ui', color: 'rgba(255,255,255,.85)' }}>{r.views}</span></div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* 친구들 후기 */}
        <div style={{ marginTop: 18, padding: '0 20px', font: '800 18px/1 Pinmoa, system-ui', letterSpacing: '-.4px', color: '#fff' }}>친구들 후기</div>
            ...
            return (
              <div key={i} className="pm-tap" onClick={() => go('friendLog', { log: { who: s.who, when: s.when, place: place.name, area: place.area, photo: s.g, caption: s.text, action: '다녀왔어요', wantBy: '나 외 1명 가고싶어해요', wantAvatars: ['me', 'doyoon'] }, space })} style={{ display: 'flex', alignItems: 'center', gap: 13, background: '#18181B', borderRadius: 18, padding: '11px 13px' }}>
                <div style={{ width: 54, height: 54, flexShrink: 0, borderRadius: '50%', padding: 2.5, background: 'conic-gradient(from 210deg,#f0a868,#d76a8a,#a83a5a,#f0a868)' }}>
                  <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '2.5px solid #18181B', background: who.color, display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 14px/1 Pinmoa, system-ui', color: '#fff' }}>{who.initial}</div>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}><span style={{ font: '700 14px/1 Pinmoa, system-ui', color: '#fff' }}>{who.name}</span><span style={{ font: '600 10.5px/1 Pinmoa, system-ui', color: '#6a6a70' }}>{s.when}</span></div>
                  <div style={{ marginTop: 5, font: "700 18px/1.05 Pinmoa, system-ui", color: '#fff' }}>{s.text}</div>
                </div>
