  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0D0D0F', overflow: 'hidden' }}>
      ...
      <div className="pm-scroll" style={{ position: 'absolute', top: 178, left: 0, right: 0, bottom: 0 }}>
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ font: '800 26px/1.1 system-ui', letterSpacing: '-.7px', color: '#fff' }}>{name}</div>
              <div style={{ marginTop: 8, font: '500 13px/1 system-ui', color: '#6a6a70' }}>{category}</div>
            </div>
          ...
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="pm-tap" onClick={() => showToast(savedCount ? `${savedCount}명이 저장: ${saverList.map((s) => s.nickname).slice(0, 3).join(', ')}` : '아직 저장한 사람이 없어요')} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#18181B', borderRadius: 13, padding: '9px 12px' }}>
              ...
              <span style={{ font: '700 12px/1 system-ui', color: '#fff' }}>{savedCount}명 저장</span>
            </div>
          </div>

          {address && (
            <div style={{ marginTop: 11, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ font: '600 13px/1 system-ui' }}>📍</span><span style={{ font: '500 13px/1.3 system-ui', color: 'rgba(255,255,255,.7)' }}>{address}</span></div>
          )}
          ...
          <div className="pm-tap" onClick={openKakao} style={{ marginTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#18181B', borderRadius: 14, padding: '13px 0' }}>
            ...
            <span style={{ font: '700 14px/1 system-ui', color: '#fff' }}>카카오맵에서 보기</span>
          </div>
        </div>
        ...
        {/* 친구들 후기 */}
        <div style={{ marginTop: 18, padding: '0 20px', font: '800 18px/1 system-ui', letterSpacing: '-.4px', color: '#fff' }}>친구들 후기 {reviews.length > 0 && <span style={{ color: '#2997ff' }}>{reviews.length}</span>}</div>
        <div style={{ marginTop: 13, padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 11 }}>
          {reviews.length === 0 ? (
            <div style={{ background: '#18181B', borderRadius: 18, padding: '20px 16px', textAlign: 'center', font: '500 13px/1.5 system-ui', color: '#6a6a70' }}>
              ...
              return (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 13, background: '#18181B', borderRadius: 18, padding: '11px 13px' }}>
                  <div style={{ width: 54, height: 54, flexShrink: 0, borderRadius: '50%', padding: 2.5, background: 'conic-gradient(from 210deg,#f0a868,#d76a8a,#a83a5a,#f0a868)' }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', border: '2.5px solid #18181B', background: memberColor(r.userId), display: 'flex', alignItems: 'center', justifyContent: 'center', font: '800 13px/1 system-ui', color: '#fff' }}>{memberInitial(r.nickname)}</div>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}><span style={{ font: '700 14px/1 system-ui', color: '#fff' }}>{r.nickname || '회원'}</span><span style={{ font: '600 10.5px/1 system-ui', color: '#6a6a70' }}>{timeAgo(r.createdAt)}</span></div>
                    <div style={{ marginTop: 5, font: "700 18px/1.05 'Caveat',cursive", color: '#fff' }}>{r.content}</div>
                  </div>
