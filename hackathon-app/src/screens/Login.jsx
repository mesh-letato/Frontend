  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 12%,#16263f,#0D0D0F 60%)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px' }}>
        ...
        <div style={{ marginTop: 20, font: '800 36px/1 system-ui', letterSpacing: '-1.4px', color: '#fff' }}>핀모아</div>
        <div style={{ marginTop: 12, textAlign: 'center', font: '500 14px/1.55 system-ui', color: 'rgba(255,255,255,.6)' }}>릴스로 발견한 장소를<br />친구들과 한 지도에 모아요</div>
        ...
        <div style={{ width: '100%', marginTop: 30, display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          {error && <div style={{ font: '600 12.5px/1.4 system-ui', color: '#ff7a7a', paddingLeft: 4 }}>{error}</div>}
        </div>
      </div>
      ...
      <div style={{ padding: '0 24px 30px' }}>
        <div className="pm-tap" onClick={submit} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: valid && !loading ? '#fff' : '#2a2a2e', borderRadius: 16, padding: 16, transition: 'background .2s', boxShadow: valid ? '0 10px 24px rgba(0,0,0,.4)' : 'none' }}>
          <span style={{ font: '800 16px/1 system-ui', color: valid && !loading ? '#0D0D0F' : '#6a6a70' }}>{loading ? '로그인 중…' : '로그인'}</span>
        </div>
        <div className="pm-tap" onClick={() => go('signup')} style={{ marginTop: 16, textAlign: 'center', font: '500 13px/1.5 system-ui', color: '#8d8d95' }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 13, padding: '0 16px' }}>
      <input
        ...
        type={type}
        style={{ flex: 1, background: 'transparent', border: 'none', padding: '15px 0', font: '500 15px/1 system-ui', color: '#fff', outline: 'none' }}
      />
    </div>
  );
}
