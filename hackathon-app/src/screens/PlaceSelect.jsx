    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0D0D0F', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(41,151,255,.18)', borderRadius: 9999, padding: '6px 12px' }}>
          ...
          <span style={{ font: '700 12px/1 system-ui', color: '#2997ff' }}>장소 3곳 찾음</span>
        </div>
      ...
      <div className="pm-scroll" style={{ flex: 1, paddingBottom: 150 }}>
        ...
        <div style={{ padding: '6px 20px 0', font: '500 14px/1.35 system-ui', color: 'rgba(255,255,255,.5)' }}>영상에서 <span style={{ color: '#2997ff', fontWeight: 800 }}>3곳</span>을 찾았어요. 맞는 곳을 골라주세요.</div>
            ...
            <div key={c.id} className="pm-tap" onClick={() => setSel(c.id)} style={{ margin: '16px 20px 0', background: '#18181B', borderRadius: 22, border: on ? '2px solid #2997ff' : '2px solid transparent', padding: 14, boxShadow: on ? '0 8px 22px rgba(41,151,255,.2)' : 'none' }}>
              <div style={{ display: 'flex', gap: 13 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ font: '800 17px/1.1 system-ui', letterSpacing: '-.4px', color: '#fff' }}>{c.name}</span><span style={{ font: '500 11px/1 system-ui', color: '#6a6a70' }}>{c.cat}</span></div>
                  <div style={{ marginTop: 5, font: '500 12.5px/1.3 system-ui', color: '#6a6a70' }}>{c.addr}</div>
                  {c.best && <div style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(48,209,88,.15)', borderRadius: 9999, padding: '4px 9px' }}><div style={{ width: 6, height: 6, borderRadius: '50%', background: '#30d158' }} /><span style={{ font: '700 11px/1 system-ui', color: '#30d158' }}>가장 정확한 후보예요</span></div>}
                </div>
                <Radio on={on} />
              </div>
            </div>
          ...
          {placeCandidates.slice(1).map((c, i) => {
            ...
            return (
              <div key={c.id} className="pm-tap" onClick={() => setSel(c.id)} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '13px 14px', borderBottom: i === 0 ? '.5px solid rgba(255,255,255,.06)' : 'none', background: on ? 'rgba(41,151,255,.08)' : 'transparent' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ font: '700 15px/1.1 system-ui', color: '#fff' }}>{c.name}</div><div style={{ marginTop: 3, font: '500 12px/1 system-ui', color: '#6a6a70' }}>{c.addr}</div></div>
                <Radio on={on} small />
              </div>
            );
          })}
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '14px 20px 30px', background: 'linear-gradient(180deg,rgba(13,13,15,0),#0D0D0F 38%)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 11 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#18181B', borderRadius: 9999, padding: '7px 12px' }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M12 21s-7-5.5-7-11a4.5 4.5 0 0 1 7-3.7A4.5 4.5 0 0 1 19 10c0 5.5-7 11-7 11Z" fill="#2997ff" /></svg><span style={{ font: '700 13px/1 system-ui', color: '#fff' }}>내 스페이스</span><svg width="11" height="7" viewBox="0 0 11 7"><path d="M1 1l4.5 4.5L10 1" stroke="#6a6a70" strokeWidth="1.6" fill="none" strokeLinecap="round" /></svg></div>
        </div>
        <div className="pm-tap" onClick={save} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#fff', borderRadius: 18, padding: 17, boxShadow: '0 10px 24px rgba(0,0,0,.4)' }}>
          ...
          <span style={{ font: '800 16px/1 system-ui', color: '#0D0D0F' }}>내 스페이스에 저장</span>
        </div>
      </div>
      <HomeIndicator />
    </div>
