    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0b0b0d', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ margin: '16px 20px 0', background: 'rgba(255,255,255,.05)', borderRadius: 20, padding: 18, display: 'flex', gap: 13, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ font: '700 14px/1.3 system-ui', color: '#fff' }}>영상에서 장소를 찾는 중</div>
          <div style={{ marginTop: 6, font: '500 12.5px/1.45 system-ui', color: 'rgba(255,255,255,.55)' }}>캡션·해시태그를 분석해 장소를 추출하고 있어요</div>
        </div>
      </div>
      ...
      <div style={{ margin: '20px 20px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ font: '700 15px/1 system-ui', color: error ? '#ff7a7a' : '#fff' }}>{error ? '분석 실패' : '장소를 불러오는 중'}</span>
        </div>
        <div style={{ marginTop: 10, font: '500 14px/1.4 system-ui', color: 'rgba(255,255,255,.5)' }}>{error || 'AI가 영상을 분석하고 있어요. 최대 30초 정도 걸려요.'}</div>
        {error ? (
          <div className="pm-tap" onClick={back} style={{ marginTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', borderRadius: 14, padding: 14 }}>
            <span style={{ font: '800 15px/1 system-ui', color: '#0D0D0F' }}>돌아가기</span>
          </div>
        ) : (
